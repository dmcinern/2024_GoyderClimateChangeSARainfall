load_raw_data = function(site,instrument,load_raw_RData,data_dirname,rData_dirname){
  ...
  if (load_raw_RData){
    ...
    raw_data = load_raw_data_core(instrument,site,data_dirname)
    if (!is.null(raw_data)){
...
load_raw_data_core = function(instrument,site,data_dirname){
  ...
  if (instrument=='AWS'){
    ...
    Hr1 = d[4]
    Min1 = d[5]
    date1 = as.POSIXct(paste0(Yr1,'/',Mon1,'/',Day1,' ',Hr1,':',Min1),format ='%Y/%m/%d %H:%M',tz='UTC')
    ...
    #
    # date = as.POSIXct(paste0(Yr,'/',Mon,'/',Day,' ',Hr,':',Min),format ='%Y/%m/%d %H:%M',tz='UTC')
