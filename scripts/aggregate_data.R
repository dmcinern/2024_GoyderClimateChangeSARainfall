aggregate_data = function(processed_data,aggPeriod,load_agg_RData,
                          agg_data_thresh){
  ...
  agg_RData_fname = strsplit(processed_data$processed_RData_fname,'.RData',fixed=T)[[1]]
  ...
  if (load_agg_RData){
    ...
    agg_data = aggregate_data_core(processed_data,aggPeriod,
                                   agg_data_thresh)
    if (!is.null(agg_data)){
      agg_data$agg_RData_fname = agg_RData_fname
      save(file=agg_RData_fname,agg_data)
    }
...
aggregate_data_core = function(processed_data,aggPeriod,
                               agg_data_thresh){
  ...
  if (accRatio==1){
    ...
    agg_data = list(date=d3$timePeriod,
                    P=d3$sum)
